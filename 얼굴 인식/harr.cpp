...
2. xml을 불러들인다==>해당xml 파일들은 haarcascade의 전면 얼굴(frontalface_default)과 눈(eye)에 대한 내용이 들어있습니다.
이는 opencv에서 제공해주므로 따로 만들 필요가 없습니다. 해당 xml 파일은 c:/opencv/source/data/haarcascades에 위치해 있습니다.

3. 입력 영상 image에서 다양한 크기의 객체 사각형 영역을 검출합니다(detectMultiScale).
Rect클래스를 이용해 벡터 타입으로 모든 사각형 정보를 저장합니다.
*/
void detect_face()
{
